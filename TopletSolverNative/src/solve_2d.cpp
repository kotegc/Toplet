    ...
    progress_callback_t progress_cb)
{
    try {
        ...
        std::vector<std::vector<KEntry>> elem_k(elem_count);
        {
            ...
            for (int ey = 0; ey < nely; ey++) {
                ...
                for (int i = 0; i < 8; i++) {
                    ...
                    for (int j = 0; j < 8; j++) {
                        const int fj = dof_to_free[edofs[j]]; if (fj < 0) continue;
                        int lo = outer[fj], hi = outer[fj + 1];
                        int pos = (int)(std::lower_bound(inner + lo, inner + hi, fi) - inner);
        ...
        typedef Eigen::ConjugateGradient<
            ...
            Eigen::IncompleteCholesky<double>> CgIc;
        CgIc cg;
        cg.setTolerance(1e-3);
        cg.setMaxIterations(200);
        cg.analyzePattern(Kff);
