...
namespace Toplet_v0_Alpha
{
    public class Toplet3DCommand : Command
    {
        ...
        protected override Result RunCommand(RhinoDoc doc, RunMode mode)
        {
            ...
            // Round up to next multiple of 8 so the GMG hierarchy can coarsen
            // exactly 3 times (2^3 = 8) without fractional element counts.
            int nelx = Math.Max(8, (((int)Math.Ceiling(width  / cellSize) + 7) / 8) * 8);
            int nely = Math.Max(8, (((int)Math.Ceiling(depth  / cellSize) + 7) / 8) * 8);
            int nelz = Math.Max(8, (((int)Math.Ceiling(height / cellSize) + 7) / 8) * 8);
